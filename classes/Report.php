...
class Report {
	...
	function init($params) {
		...
		if(!empty($errs)) {
			Fatal::internalError(T('ReportInitReport', array('error'=>GeneralError::listToStr($errs))));
		}
	}
	...
	function variant($newParams, $newName=NULL) {
		...
		if(!empty($errs)) {
			Fatal::internalError(T('ReportMakingVariant', array('error'=>GeneralError::listToStr($errs))));
		}
		return $rpt;
	}
